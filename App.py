...
st.title('RESUME CLASSIFICATION  MODEL DEPLOYMENT GROUP-6')
...
def extract_skills(resume_text):
    ...
    tokens = [token.text.lower() for token in nlp_text if not token.is_stop]  # Lowercase and remove stop words

    data = pd.read_csv(r"skills.csv")  # Assuming the skills are in the first column of the CSV
    skills = list(data.columns.values)  # Extracting skills list
    skills = [skill.lower() for skill in skills]  # Lowercase for uniformity

    skillset = set()

    for token in tokens:  # Check one-grams
        if token in skills:
            skillset.add(token)

    for chunk in noun_chunks:  # Check for multi-grams (e.g., 'machine learning')
        chunk_text = chunk.text.lower().strip()
        if chunk_text in skills:
            skillset.add(chunk_text)

    return [skill.capitalize() for skill in skillset]  # Returning a capitalized, unique set of skills
...
# MAIN CODE
import pickle as pk
model = pk.load(open(r'model_RF.pkl', 'rb'))
